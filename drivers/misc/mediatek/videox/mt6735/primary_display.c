...
int checked_times_since_boot = 0;
extern void unmask_hal_eint(void);
extern int status_matched;
extern int lcd_b;

//extern int status_lk;
//extern int status_first;
//extern int status_after;
//extern char * lcm_status_p;
//-------------------------------------------------------
...
extern int backlight_lock;
int primary_display_esd_check(void)
{
	int ret = 0;
        if(backlight_lock == 1) 
        {
            DISPCHECK("[ESD]backlight_lock is true, ESD check return\n");
            return ret;
        }
	_primary_path_esd_check_lock();
	...
	DISPCHECK("[ESD]ESD check begin: %d. \n",status_matched);
	//DISPCHECK("[ESD]ESD check begin: %d, %d, %d, %d. \n", checked_times_since_boot, status_first, status_lk, status_matched);
    _primary_path_lock(__func__);
	if(primary_get_state() != DISP_ALIVE)
	{
		MMProfileLogEx(ddp_mmp_get_events()->esd_check_t, MMProfileFlagPulse, 1, 0);
...
//---- Kevin Pi add to handle fliping action soon after boot ------
/*
		if(checked_times_since_boot<1)
			checked_times_since_boot++;
		if(checked_times_since_boot==1)
		{
			if(status_matched == 0)
			{
...
int primary_display_esd_recovery_test(void)  //by Colin
{
...
//
	...
	//disp_lcm_suspend(pgc->plcm);  //touch invalid
	...
	DISPCHECK("[ESD]start dpmgr path[begin]\n");
	dpmgr_path_start(pgc->dpmgr_handle, CMDQ_DISABLE);  //must
	DISPCHECK("[ESD]start dpmgr path[end]\n");
	if(dpmgr_path_is_busy(pgc->dpmgr_handle))
	{
	...
	if(primary_display_is_video_mode())
	{
		...
		dpmgr_path_trigger(pgc->dpmgr_handle, NULL, CMDQ_DISABLE);  //must
	} 
...
int primary_display_esd_recovery(void)
{
	...
	DISPCHECK("[POWER]lcm suspend[begin]\n");
	//disp_lcm_suspend(pgc->plcm);  //by Colin
	DISPCHECK("[POWER]lcm suspend[end]\n");
...
//---- Kevin Pi add to handle fliping action soon after boot ------
    ...
    if(lcd_b == 0){
		mt_set_gpio_mode((GPIO69 | 0x80000000), GPIO_MODE_GPIO);
		mt_set_gpio_dir((GPIO69 | 0x80000000), GPIO_DIR_OUT);
		mt_set_gpio_out((GPIO69 | 0x80000000), GPIO_OUT_ONE);
