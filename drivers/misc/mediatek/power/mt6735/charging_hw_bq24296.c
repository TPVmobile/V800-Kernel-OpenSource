...
static kal_uint32 charging_hw_init(void *data)
{
	...
	#endif
	mt_set_gpio_mode(GPIO_CHG_EN_PIN, GPIO_CHG_EN_PIN_M_GPIO);
	mt_set_gpio_dir(GPIO_CHG_EN_PIN, GPIO_DIR_OUT);
	mt_set_gpio_out(GPIO_CHG_EN_PIN, GPIO_OUT_ZERO);  
	msleep(10); 
...
//pengguangyi add 20151119 begin
		bootmode = get_boot_mode();
		if ((bootmode == META_BOOT) || (bootmode == ADVMETA_BOOT))
				bq24296_set_en_hiz(0x1);	// disable power path
//end
		printk("[charging_enable] bq24298_hiz_en = %d\n", bq24296_hiz_en);
		if(bq24296_hiz_en) bq24296_set_en_hiz(0x1);
...
//pengguangyi add 20151202 begin
    if(current_value >= CHARGE_CURRENT_1500_00_MA)
    {
        register_value = 29;
    }
    else
