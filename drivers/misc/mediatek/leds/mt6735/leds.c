...
int g_ro_level=100;
extern int lcd_b;
extern int backlight_lock;//AAL Service will set the backlight, so use the flag
int mt_mt65xx_led_set_cust(struct cust_mt65xx_led *cust, int level)
{
	...
	*/	
    ...
    LEDS_DEBUG("set brightness, name:%s, mode:%d, level:%d\n", cust->name, cust->mode, level);

/*********add by Colin 20151022*********/
    if((strcmp(cust->name,"lcd-backlight") == 0) ||(strcmp(cust->name,"lcd-backlight_b") == 0))
    {	
        if(backlight_lock == 1) 
        {
            g_ro_level = level;
            printk("lcd-backlight: backlight_lock is 1, set level 0\n");
            level = 0;
/*            
            mt_set_gpio_mode((GPIO55 | 0x80000000), GPIO_MODE_GPIO);  
            mt_set_gpio_dir((GPIO55 | 0x80000000), GPIO_DIR_OUT);
            mt_set_gpio_out((GPIO55 | 0x80000000), GPIO_OUT_ZERO);  //Led_b Off 
            mt_set_gpio_mode((GPIO69 | 0x80000000), GPIO_MODE_GPIO);  
            mt_set_gpio_dir((GPIO69 | 0x80000000), GPIO_DIR_OUT);
            mt_set_gpio_out((GPIO69 | 0x80000000), GPIO_OUT_ZERO);  //Led_a Off
            return -1;
            */
        }
        if(lcd_b == 0){
            printk(KERN_ERR"----Colin--lcd-backlight -- ---- \n");
            mt_set_gpio_mode((GPIO69 | 0x80000000), GPIO_MODE_01); //LCD_a  ---01 BLS_PWM
            mt_set_gpio_dir((GPIO69 | 0x80000000), GPIO_DIR_OUT);
        ...
        {	
            printk(KERN_ERR"----Colin--lcd-backlight_b -- ---- \n");
            mt_set_gpio_mode((GPIO55 | 0x80000000), GPIO_MODE_07); //LCD_b  ---07 BLS_PWM
            mt_set_gpio_dir((GPIO55 | 0x80000000), GPIO_DIR_OUT);
...
/*********add by Colin 20151022*********/
	...
	switch (cust->mode) {
		...
		case MT65XX_LED_MODE_PMIC:
            ...
            else if((strcmp(cust->name,"keyboard-backlight") == 0))
            {
                int ret1,ret2;
                ret1 = mt_brightness_set_pmic(MT65XX_LED_PMIC_NLED_ISINK0, level, bl_div_hal);
                ret2 = mt_brightness_set_pmic(MT65XX_LED_PMIC_NLED_ISINK1, level, bl_div_hal);
...
//======================================
int green_want_wake2blink = 0;
int green_sleep_on = 0;
int green_sleep_off = 0;

int red_want_wake2blink = 0;
int red_sleep_on = 0;
int red_sleep_off = 0;

int blue_want_wake2blink = 0;
int blue_sleep_on = 0;
int blue_sleep_off = 0;

void led_stop_blink(void)
{
	green_want_wake2blink = 0;
	green_sleep_on = 0;
	green_sleep_off = 0;
	red_want_wake2blink = 0;
	red_sleep_on = 0;
	red_sleep_off = 0;
	blue_want_wake2blink = 0;
	blue_sleep_on = 0;
	blue_sleep_off = 0;
}
//======================================
	...
	if (*delay_on != led_data->delay_on || *delay_off != led_data->delay_off) {
		...
		if (led_data->delay_on && led_data->delay_off) { // enable blink
				 ...
				 || led_data->cust.data == MT65XX_LED_PMIC_NLED_ISINK3))
			{
				...
				if(strcmp(led_data->cust.name, "green") == 0)
				{
					green_want_wake2blink = 1;
				...
				else if(strcmp(led_data->cust.name, "red") == 0)
				{
					red_want_wake2blink = 1;
					if(*delay_on > 0 && *delay_on < 1000) red_sleep_on = 1;
					else
					red_sleep_on = *delay_on/1000;
					if(*delay_off > 0 && *delay_off < 1000) red_sleep_off = 1;
					else
					red_sleep_off = *delay_off/1000;
				}
				else if(strcmp(led_data->cust.name, "blue") == 0)
				{
					blue_want_wake2blink = 1;
					if(*delay_on > 0 && *delay_on < 1000) blue_sleep_on = 1;
					else
					blue_sleep_on = *delay_on/1000;
					if(*delay_off > 0 && *delay_off < 1000) blue_sleep_off = 1;
					else
					blue_sleep_off = *delay_off/1000;
				}
			}
