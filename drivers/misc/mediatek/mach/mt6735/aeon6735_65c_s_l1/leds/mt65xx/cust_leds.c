...
#define GPIO_BUTTON_BLACKLIGHT_EN_PIN (GPIO85 | 0X80000000)
#define GPIO_BUTTON_BLACKLIGHT_M GPIO_MODE_00
...
int cust_set_ledfuc_b(unsigned int level)
{
	printk("cust_set_ledfuc_b level=%d\n",level);
	if(level == 0)
	{
		blue_led_state=0;
	    mt_set_gpio_mode(GPIO_LED_B_EN_PIN,GPIO_LED_B_EN_PIN_M_GPIO);  
	    mt_set_gpio_dir(GPIO_LED_B_EN_PIN,GPIO_DIR_OUT);
	    mt_set_gpio_out(GPIO_LED_B_EN_PIN,GPIO_OUT_ZERO);		
	}
	else
	{
		blue_led_state=1;
	    mt_set_gpio_mode(GPIO_LED_B_EN_PIN,GPIO_LED_B_EN_PIN_M_GPIO);  
	    mt_set_gpio_dir(GPIO_LED_B_EN_PIN,GPIO_DIR_OUT);
	    mt_set_gpio_out(GPIO_LED_B_EN_PIN,GPIO_OUT_ONE);
	}
}

int green_led_state = 0;
int cust_set_ledfuc_g(unsigned int level)
{
	printk("cust_set_ledfuc_g level=%d\n",level);
	if(level == 0)
	{
		green_led_state=0;
	    mt_set_gpio_mode(GPIO_LED_G_EN_PIN,GPIO_LED_G_EN_PIN_M_GPIO);  
	    mt_set_gpio_dir(GPIO_LED_G_EN_PIN,GPIO_DIR_OUT);
	    mt_set_gpio_out(GPIO_LED_G_EN_PIN,GPIO_OUT_ZERO);		
	}
	else
	{
		green_led_state=1;
	    mt_set_gpio_mode(GPIO_LED_G_EN_PIN,GPIO_LED_G_EN_PIN_M_GPIO);  
	    mt_set_gpio_dir(GPIO_LED_G_EN_PIN,GPIO_DIR_OUT);
	    mt_set_gpio_out(GPIO_LED_G_EN_PIN,GPIO_OUT_ONE);
	}
}

int red_led_state = 0;
int cust_set_ledfuc_r(unsigned int level)
{
	printk("cust_set_ledfuc_r level=%d\n",level);
	if(level == 0)
	{
		red_led_state=0;
        mt_set_gpio_mode(GPIO_LED_R_EN_PIN,GPIO_LED_R_EN_PIN_M_GPIO);  
	    mt_set_gpio_dir(GPIO_LED_R_EN_PIN,GPIO_DIR_OUT);
	    mt_set_gpio_out(GPIO_LED_R_EN_PIN,GPIO_OUT_ZERO);		
	}
	else
	{
		red_led_state=1;
	    mt_set_gpio_mode(GPIO_LED_R_EN_PIN,GPIO_LED_R_EN_PIN_M_GPIO);  
	    mt_set_gpio_dir(GPIO_LED_R_EN_PIN,GPIO_DIR_OUT);
	    mt_set_gpio_out(GPIO_LED_R_EN_PIN,GPIO_OUT_ONE);
	}
}


int cust_set_button_bl(unsigned int level)
{
	printk("cust_set_button_bl level=%d\n",level);
	if(level == 0)
	{
        mt_set_gpio_mode(GPIO_BUTTON_BLACKLIGHT_EN_PIN,GPIO_BUTTON_BLACKLIGHT_M);  
	    mt_set_gpio_dir(GPIO_BUTTON_BLACKLIGHT_EN_PIN,GPIO_DIR_OUT);
	    mt_set_gpio_out(GPIO_BUTTON_BLACKLIGHT_EN_PIN,GPIO_OUT_ZERO);		
	}
	else
	{
	    mt_set_gpio_mode(GPIO_BUTTON_BLACKLIGHT_EN_PIN,GPIO_BUTTON_BLACKLIGHT_M);  
	    mt_set_gpio_dir(GPIO_BUTTON_BLACKLIGHT_EN_PIN,GPIO_DIR_OUT);
	    mt_set_gpio_out(GPIO_BUTTON_BLACKLIGHT_EN_PIN,GPIO_OUT_ONE);
	}
}
/*
...
static struct cust_mt65xx_led cust_led_list[MT65XX_LED_TYPE_TOTAL] = {
    {"red",               MT65XX_LED_MODE_GPIO, (long)cust_set_ledfuc_r,{0}},
    {"green",             MT65XX_LED_MODE_GPIO, (long)cust_set_ledfuc_g,{0}},
    {"blue",              MT65XX_LED_MODE_GPIO, (long)cust_set_ledfuc_b,{0}},
	{"jogball-backlight", MT65XX_LED_MODE_NONE, -1,{0}},
	{"keyboard-backlight",MT65XX_LED_MODE_PMIC, -1,{0}},
	{"button-backlight",  MT65XX_LED_MODE_GPIO, (long)cust_set_button_bl,{0}},
	{"lcd-backlight",     MT65XX_LED_MODE_CUST_BLS_PWM, (long)disp_bls_set_backlight,{0}},
    {"lcd-backlight_b",     MT65XX_LED_MODE_CUST_BLS_PWM, (long)disp_bls_set_backlight,{0}},
};
