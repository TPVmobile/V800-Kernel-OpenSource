...
bool Speaker_Init(void)
{
   ...
   mt_set_gpio_mode(GPIO_PA_EN_PIN,GPIO_MODE_00);  // gpio mode
   mt_set_gpio_pull_enable(GPIO_PA_EN_PIN,GPIO_PULL_ENABLE);
   mt_set_gpio_dir(GPIO_PA_EN_PIN,GPIO_DIR_OUT); // output
   mt_set_gpio_out(GPIO_PA_EN_PIN,GPIO_OUT_ONE); // high
   PRINTK("-Speaker_Init Success");
   return true;
}
...
void Sound_Speaker_Turnon(int channel)
{
    ...
    if(gsk_on)
        return;
    mt_set_gpio_dir(GPIO_PA_EN_PIN,GPIO_DIR_OUT); // output
    mt_set_gpio_out(GPIO_PA_EN_PIN,GPIO_OUT_ONE); // low
...
void Sound_Speaker_Turnoff(int channel)
{
	...
	if(!gsk_on)
		return;
    mt_set_gpio_dir(GPIO_PA_EN_PIN,GPIO_DIR_OUT); // output
    mt_set_gpio_out(GPIO_PA_EN_PIN,GPIO_OUT_ZERO); // high
	gsk_on = false;
}
