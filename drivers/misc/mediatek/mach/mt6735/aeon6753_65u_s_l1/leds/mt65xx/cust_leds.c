...
#define GPIO_LED_EN2_PIN GPIO_HALL_4_PIN 
#define GPIO_LED_EN2_PIN_M_GPIO GPIO_HALL_4_PIN_M_GPIO

int green_led_state = 0;
int cust_set_ledfuc_green(unsigned int level)
{
	printk("cust_set_ledfuc level=%d\n",level);
	if(level == 0)
	{
		green_led_state = 0;
...
int red_led_state = 0;
int cust_set_ledfuc_red(unsigned int level)
{
	printk("cust_set_led2fuc level=%d\n",level);
	if(level == 0)
	{
		red_led_state = 0;
		mt_set_gpio_mode(GPIO_LED_EN2_PIN,GPIO_LED_EN2_PIN_M_GPIO);  
	    mt_set_gpio_dir(GPIO_LED_EN2_PIN,GPIO_DIR_OUT);
	    mt_set_gpio_out(GPIO_LED_EN2_PIN,GPIO_OUT_ZERO);		
	}
	else
	{
		red_led_state = 1;
		mt_set_gpio_mode(GPIO_LED_EN2_PIN,GPIO_LED_EN2_PIN_M_GPIO);  
	    mt_set_gpio_dir(GPIO_LED_EN2_PIN,GPIO_DIR_OUT);
	    mt_set_gpio_out(GPIO_LED_EN2_PIN,GPIO_OUT_ONE);
	}
}
...
static struct cust_mt65xx_led cust_led_list[MT65XX_LED_TYPE_TOTAL] = {
	{"red",               MT65XX_LED_MODE_GPIO, (long)cust_set_ledfuc_red,{0}},
	{"green",             MT65XX_LED_MODE_GPIO, (long)cust_set_ledfuc_green,{0}},
