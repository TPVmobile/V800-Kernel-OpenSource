...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id) 
{
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			if (*sensor_id == imgsensor_info.sensor_id) {				
				LOG_INF("lanhai ov5670 i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);	  
				return ERROR_NONE;
			}	
			LOG_INF("lanhai Read ov5670 i2c write id fail, id: 0x%x sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
	...
	LOG_INF("ov5670 id OK, id: 0x%x 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
	if (*sensor_id != imgsensor_info.sensor_id) {
