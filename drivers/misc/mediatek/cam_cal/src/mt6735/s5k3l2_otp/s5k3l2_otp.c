...
*
********************************************************************************/
#define CAM_CAL_DRVNAME "S5K3L2_CAM_CAL_DRV"
...
int S5K3L2_iWriteCAM_CAL(u16 a_u2Addr  , u32 a_u4Bytes, u8 puDataInBytes)
{
    int  i4RetValue = 0;
    u32 u4Index = 0;
	int retry = 3;
	...
	do {
        CAM_CALDB("[CAM_CAL][S5K3L2_iWriteCAM_CAL] Write 0x%x=0x%x \n",a_u2Addr, puDataInBytes);
   ...
   //CAM_CALDB("[CAM_CAL] S5K3L2_iWriteCAM_CAL done!! \n");
    return 0;
}


//Address: 2Byte, Data: 1Byte
int S5K3L2_iReadCAM_CAL(u8 a_u2Addr, u32 ui4_length, u8 * a_puBuff)
{
    int  i4RetValue = 0;
    char puReadCmd[1] = {(char)(a_u2Addr)};

    //CAM_CALDB("[CAM_CAL] S5K3L2_iReadCAM_CAL!! \n");   
	...
	CAM_CALDB("[CAM_CAL][S5K3L2_iReadCAM_CAL] Read 0x%x=0x%x \n", a_u2Addr, a_puBuff[0]);
    ...
    //CAM_CALDB("[CAM_CAL] S5K3L2_iReadCAM_CAL done!! \n");
    return 0;
}
//*/
//Burst Write Data
static int iWriteData(unsigned int  ui4_offset, unsigned int  ui4_length, unsigned char * pinputdata)
{
   ...
   do 
   {
       ...
       i4RetValue = S5K3L2_iWriteCAM_CAL((u16)u4CurrentOffset, 1, pBuff[0]);
       if (i4RetValue != 0)
       {
...
/*   if (ui4_offset + ui4_length >= 0x2000)
   ...
   */
   ...
   do 
   {
       i4RetValue = S5K3L2_iReadCAM_CAL((u8)u4CurrentOffset, 1, pBuff);
...
static long Ioctl_Compat(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    stCAM_CAL_INFO_STRUCT __user *data;
    int err;
	  CAM_CALDB("[CAMERA SENSOR] Ioctl_Compat,%p %p %x ioc size %d\n",filp->f_op ,filp->f_op->unlocked_ioctl,cmd,_IOC_SIZE(cmd) );
...
#endif
{
    ...
    int i,temp1;
    kal_int8 index = -1; //for check otp page
    ...
    switch(a_u4Command)
    {
        ...
        case CAM_CALIOC_G_READ:
			...
			if(ptempbuf->u4Length > 256) // read LSC data, 932 bytes
			{
				g_pstI2Cclient->addr = ((S5K3L2OTP_DEVICE_ID+2)>>1); //read block1
				CAM_CALDB("[CAM_CAL] i2c address = 0x%02x\n", g_pstI2Cclient->addr);
				i4RetValue = iReadData((u8)(ptempbuf->u4Offset+OTP_START_ADDR), 256, pWorkingBuff);
				
				g_pstI2Cclient->addr = ((S5K3L2OTP_DEVICE_ID+4)>>1); //read block2
				CAM_CALDB("[CAM_CAL] i2c address = 0x%02x\n", g_pstI2Cclient->addr);
				i4RetValue = iReadData((u8)(ptempbuf->u4Offset+OTP_START_ADDR), 256, pWorkingBuff+0x100);
				
				g_pstI2Cclient->addr = ((S5K3L2OTP_DEVICE_ID+6)>>1); //read block3
				CAM_CALDB("[CAM_CAL] i2c address = 0x%02x\n", g_pstI2Cclient->addr);
				i4RetValue = iReadData((u8)(ptempbuf->u4Offset+OTP_START_ADDR), 256, pWorkingBuff+0x200);
				
				g_pstI2Cclient->addr = ((S5K3L2OTP_DEVICE_ID+8)>>1); //read block4
				CAM_CALDB("[CAM_CAL] i2c address = 0x%02x\n", g_pstI2Cclient->addr);
				i4RetValue = iReadData((u8)(ptempbuf->u4Offset+OTP_START_ADDR), 256, pWorkingBuff+0x300);
				
				g_pstI2Cclient->addr = ((S5K3L2OTP_DEVICE_ID+0x0A)>>1); //read block4
				CAM_CALDB("[CAM_CAL] i2c address = 0x%02x\n", g_pstI2Cclient->addr);
				i4RetValue = iReadData((u8)(ptempbuf->u4Offset+OTP_START_ADDR), 256, pWorkingBuff+0x400);
				
				g_pstI2Cclient->addr = ((S5K3L2OTP_DEVICE_ID+0x0D)>>1); //read block4
				CAM_CALDB("[CAM_CAL] i2c address = 0x%02x\n", g_pstI2Cclient->addr);
				i4RetValue = iReadData((u8)(ptempbuf->u4Offset+OTP_START_ADDR), 256, pWorkingBuff+0x500);
				
				g_pstI2Cclient->addr = ((S5K3L2OTP_DEVICE_ID+0x0E)>>1); //read block4
				CAM_CALDB("[CAM_CAL] i2c address = 0x%02x\n", g_pstI2Cclient->addr);
				i4RetValue = iReadData((u8)(ptempbuf->u4Offset+OTP_START_ADDR), 100, pWorkingBuff+0x600);
...
inline static int RegisterCAM_CALCharDrv(void)
{
    ...
    CAM_CAL_class = class_create(THIS_MODULE, "S5K3L2_CAM_CALdrv");
