...
static unsigned char CTPM_FW[] = {
	#include "5x46_Ref_S616_ZHENGHAI_V13_D01_20150907_app.i"
};
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	if(1)//(uc_tp_fm_ver <= uc_host_fm_ver) //  fw_id_control  uc_tp_fm_ver < uc_host_fm_ver
	{
		msleep(100);
		printk("[FTS] uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x\n",uc_tp_fm_ver, uc_host_fm_ver);
		...
		printk("[FTS] current fw is the latsest ,cancel update ~~\n");
