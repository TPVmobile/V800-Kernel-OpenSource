...
//#include "TPV_V2_V01_20150629.h"
//#include "NVT_firmware_205.h"
//#include "Truly_V1_SangFei_V15_20150818.h"
//#include "Truly_V1_SangFei_V17_20150828.h"
//#include "SangFei_V21.h"
#include "TPV_V1_SangFei_V22_20150921.h"
...
*/
	...
	for(index = 0; index < TPD_MAX_POINTS_NUM; index++)
	{
       ...
       for(index = 0; index < TPD_MAX_POINTS_NUM; index++)
       {
               if(press_id[index] != 1) /* finger up (break) */
                       tpd_up(index, 0, 0, 0, 0);
       }

       input_report_key(tpd->dev, BTN_TOUCH, (point_count>0)); /* indicate one of fingers down/up */
#else
	if(point_count == 0)
	{
		tpd_up(index, 0, 0, 0, 0);
...
#if MT_PROTOCOL_B
        input_mt_init_slots(tpd->dev, TPD_MAX_POINTS_NUM, 0);
        input_set_abs_params(tpd->dev, ABS_MT_TOOL_TYPE, 0, MT_TOOL_FINGER, 0, 0);
#endif
