...
static struct proc_dir_entry *gt91xx_wakeup_proc = NULL;
static int gt91xx_wakeup_read_proc(char *buf, char **start, off_t offset, int request, int *eof, void *data)
{      
	int len=0;
	len = sprintf(buf,"%d\n",gesture_wakeup);
	return len;     
}
static int gt91xx_wakeup_write_proc( struct file *filp, const char __user *buf,unsigned long len, void *data )
{      
	char str_buf[256]={0};
	int tmp = 0;
	copy_from_user(str_buf,buf,len);
	str_buf[len] = '\0';    
	gesture_wakeup = simple_strtol(str_buf,NULL,10);
	printk("gt91xx_wakeup_write_proc gesture_wakeup = %d\n",gesture_wakeup);
	return len; 
}
#endif

#if 1 //GTP_HALL_CLOSE_CFG
#define FCOVER_OPEN  GTP_MAX_HEIGHT
#define FCOVER_CLOSE GTP_MAX_HEIGHT-980

static u16 tpd_fcover_flag = FCOVER_OPEN;

void tpd_fcover_setting(u8 enable)
{
	if (enable == 1)
		tpd_fcover_flag = FCOVER_OPEN;
	else
		tpd_fcover_flag = FCOVER_CLOSE;
...
EXPORT_SYMBOL(tpd_fcover_setting);
#endif
...
s32 gtp_send_cfg(struct i2c_client *client)
{
    ...
    {
	#if GTP_HALL_CLOSE_CFG
        if(tpd_fcover_flag == FCOVER_OPEN)
        {
...
static s32 gtp_init_panel(struct i2c_client *client)
{
...
#if GTP_HALL_CLOSE_CFG
	u8 *send_hallclose_buf[] = {cfg_info_group0_hallclose,
                          cfg_info_group1_hallclose};
#endif
    ...
    memcpy(&config_hallclose[GTP_ADDR_LENGTH], send_hallclose_buf[sensor_id], cfg_len);
#endif
    ...
    config_hallclose[RESOLUTION_LOC + 2] = (u8)(GTP_MAX_HEIGHT-980);
    config_hallclose[RESOLUTION_LOC + 3] = (u8)((GTP_MAX_HEIGHT-980)>>8);
...
#endif
    {
        ...
        config[cfg_len] = (~check_sum) + 1;
		gtp_i2c_write(client, config, GTP_ADDR_LENGTH + 1);  //linxf for set config version
    #endif
        GTP_INFO("X_MAX = %d, Y_MAX = %d, TRIGGER = 0x%02x",
            abs_x_max,abs_y_max,int_type);
    }
...
static ssize_t gt9xx_hall_switch_read(char *buf, char **start, off_t off, int count, int *eof, void *data)
{
	return sprintf(buf,"%d\n",gtp_hall_state);
}

static ssize_t gt9xx_hall_switch_write(struct file *filp, const char *buff, unsigned long len, void *data)
{
	down(&g_hall_sem);
	if (copy_from_user(&gtp_hall_state,(int *)buff, 1))
	{
		up(&g_hall_sem);
		return -EFAULT;
	}
	gtp_hall_state = gtp_hall_state -'0';
	//tpd_fcover_setting(gtp_hall_state);   //linxf 20140714
	up(&g_hall_sem);
	return len;
}
#endif
...
//    }
#if GTP_GESTURE_WAKEUP
	gt91xx_wakeup_proc = create_proc_entry( GT91XX_WAKEUP_PROC_FILE, 0666, NULL);
	if(gt91xx_wakeup_proc)
	{	
		gt91xx_wakeup_proc->write_proc  = gt91xx_wakeup_write_proc;
		gt91xx_wakeup_proc->read_proc = gt91xx_wakeup_read_proc;
	}
	else
	{
		printk(KERN_ERR "[GESTURE] %s: proc register file create failed!\n", __func__);
	}	
#endif
#if GTP_HALL_CLOSE_CFG
	sema_init(&g_hall_sem, 1);
	gtp_wakeup_hall_state = create_proc_entry(GT91XX_HALL_PROC_FILE, 0666, NULL);
	if(gtp_wakeup_hall_state)
	{
		gtp_wakeup_hall_state->read_proc = gt9xx_hall_switch_read;
		gtp_wakeup_hall_state->write_proc = gt9xx_hall_switch_write;
	}
	else
	{
		printk(KERN_ERR "[HALL] %s: proc register file create failed!\n", __func__);
	}
#endif
	...
	input_set_capability(tpd->dev, EV_KEY, KEY_J);
...
static int touch_event_handler(void *unused)
{
	...
	#if GTP_HALL_CLOSE_CFG   //linxf 20140714
		i2c_read_bytes(i2c_client_point, GTP_REG_CONFIG_DATA, temp_data, 5);
		hall_state = (temp_data[4] << 8) + temp_data[3];
		if (hall_state != tpd_fcover_flag)
		{
        ...
        if (DOZE_ENABLED == doze_status)
        {
            ...
            {               
                if ( (doze_buf[2] == 'c') || (doze_buf[2] == 'e') || (doze_buf[2] == 'm') 
                		|| (doze_buf[2] == 'o') || (doze_buf[2] == 'w') || (doze_buf[2] == 's')
                		)
                {
                    if (doze_buf[2] != 0x5E)
                    {
					...
					switch(doze_buf[2])
					{
						...
						case 'e':
							...
							break;
						case 'm':
							input_report_key(tpd->dev, KEY_I, 1);
							input_sync(tpd->dev);
							input_report_key(tpd->dev, KEY_I, 0);
							input_sync(tpd->dev);								
							break;
						case 'o':
							...
							break;
						case 'w':
							input_report_key(tpd->dev, KEY_J, 1);
							input_sync(tpd->dev);
							input_report_key(tpd->dev, KEY_J, 0);
							input_sync(tpd->dev);										
							break;
						case 's':
							input_report_key(tpd->dev, KEY_Q, 1);
							input_sync(tpd->dev);
							input_report_key(tpd->dev, KEY_Q, 0);
							input_sync(tpd->dev);		
							break;
					}
                    // clear 0x814B
...
static s8 gtp_wakeup_sleep(struct i2c_client *client)
{
    ...
    while (retry++ < 10)
    {
        ...
        gtp_reset_guitar(client, 20);
        mt_eint_unmask(CUST_EINT_TOUCH_PANEL_NUM);
    #else  // maybe TP invalid  ---- Sanford.lin
        gtp_reset_guitar(client, 20); //GTP_GPIO_OUTPUT(GTP_INT_PORT, 1);
        msleep(5);
    #endif
