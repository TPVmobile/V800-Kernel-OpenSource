...
static kal_bool mt_battery_0Percent_tracking_check(void)
{
	...
	if (BMT_status.UI_SOC <= 0) {
		...
		*/
                if(BMT_status.ZCV <= 3480 ||  BMT_status.temperature <= 0 )
                {
                        if (BMT_status.bat_vol > SYSTEM_OFF_VOLTAGE && BMT_status.UI_SOC > 1) 
                        {
                  	 BMT_status.UI_SOC--;
...
static void mt_battery_Sync_UI_Percentage_to_Real(void)
{
	...
	if(bat_ui_timeout)
	{
		battery_log(BAT_LOG_CRTI,"bat_ui_timeout is true\n");
		timer_counter = SYNC_TO_REAL_TRACKING_TIME/BAT_TASK_PERIOD;
		bat_ui_timeout = false;
	}
...
//pengguangyi 20151126 start
		#ifdef MTK_VOLTAGE_RECHARGE_SUPPORT 
		if((BMT_status.charger_protect_status == charger_OVER_VOL) && (BMT_status.charger_vol <= 6400 ))
		{
...
void BAT_thread(void)
{
	...
	if (BMT_status.charger_exist == KAL_TRUE || bq24296_hiz_en) {
